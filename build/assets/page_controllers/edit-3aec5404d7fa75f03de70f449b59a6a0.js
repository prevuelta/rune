...
function RuneView (runeModel) {
	...
	this.drawLetter(runeModel.letter.gridPoints, runeModel.letter.selectedPoints);
...
RuneView.prototype = {
	...
	drawLetter : function(gridPoints, selected) {
...
LetterView.prototype = {
	...
	draw : function(selected) {
		...
		$.each(letter.renderedPoints, function(idx, point) {
			...
			letter.createLetterPoint(point, idx, selected.indexOf(idx));
...
TabletModelController.prototype = {
	...
	deleteSelected : function() {
		var tablet = this;
		var gridPoints = this.getActiveRune().letter.gridPoints;
		this.getActiveRune().letter.gridPoints = gridPoints.filter(function(entry, idx) {
			return tablet.selectedPoints.indexOf(idx) == -1; 
		});
	}
}
...
var eventHandlers = {
	...
	selectPoint: function(data) {
		...
		if(data[0]) {
			app.tablet.getActiveRune().selectedPoints.push(data[1]);
		} else {
			app.tablet.getActiveRune().selectedPoints = _.without(app.tablet.getActiveRune().selectedPoints, data[1]);
		}
	...
	deselectAll: function(e) {
		app.tablet.getActiveRune().selectedPoints = [];
	}
}
