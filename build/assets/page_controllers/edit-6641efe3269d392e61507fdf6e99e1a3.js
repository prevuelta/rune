...
WorkSpace.prototype = {
	...
	redrawLetter : function(letterModel) {
		this.runeView.drawLetter(letterModel.renderedPoints);
	}
}
...
RuneView.prototype = {
	...
	drawLetter : function(pointArray) {
		this.letter.draw();
	},
	drawGrid : function() {
	...
	addLetterView : function(paper) {
		this.letter = new LetterView(paper);
	}
}
...
LetterModel.prototype.clearPoints = function() {
	this.options.gridPoints = [];
}

LetterModel.prototype.addPoint= function(point) {
	this.options.gridPoints.push(point);
}

LetterModel.prototype.preRender = function(grid) {
...
LetterModel.prototype.reset = function() {
	this.gridPoints = [];
}

LetterModel.prototype.distort = function(type) {
	switch(type) {
		case "random" : 
...
function RuneEditor(options, paper) {
	...
	document.addEventListener('addPoint', function(e) {
		...
		editor.workspace.redrawLetter(activeRune.letter);
	});
