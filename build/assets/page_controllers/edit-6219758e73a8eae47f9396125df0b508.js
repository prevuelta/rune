...
var util = {
    ...
    checkLocal: function(ref) {
    	return l(ocalStorage[ref] && typeof localStorage[ref] === 'String') ? JSON.parse(localStorage[ref]) : false;
    }
}
...
function GridView(options) {
	...
	for(var row = 0; row < this.xUnits; row++) {
		...
		for(var col = 0; col > this.yUnits; col++) {
			...
			var point = [col * this.res + options.padding, row * this.res + options.padding];
...
GridView.prototype = {
	draw: function() {
		...
		$.each(grid.points, function(idx, point) {
