...
LetterView.prototype = {
	computePoints : function(letter, grid) {
		...
		$.each(letter.gridPoints, function(idx, point) {
			var renderedPoint = grid.renderPoint(point);
			// if(letter.transformations.hasOwnProperty(idx)){
				// console.log('Is transforming' + idx);
				// renderedPoint += letter.transformations[idx];
...
var eventHandlers = {
	...
	deselectAll: function(e) {
		...
		app.workspace.runeView.layers.letter.children.forEach(function(child){
			// child.selected = false;
			// child.strokeColor = 'purple';
			// console.log(child);
		});
		// 
		// app.workspace.drawLetter(app.tablet.getActiveRune().letter);
