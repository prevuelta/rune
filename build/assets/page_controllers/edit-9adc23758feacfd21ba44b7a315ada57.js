...
LetterView.prototype = {
	...
	createLetterPoint: function(point, value, selected, transform) {
		...
		if(transform)
			letterPoint.point.add((function() { var point = new paper.Point(); point.angle = transform[0]; point.length = transform[1]; return point;})());

		console.log(letterPoint.point.x);

		letterPoint.path = paper.Path.Rectangle(letterPoint.point.x - 5, letterPoint.point.y - 5, 10);

		letterPoint.path.strokeColor = 'red';
		letterPoint.path.fillColor = 'white';

		letterPoint.path.value = value;

		letterPoint.path.selected = selected || false;
		
		letterPoint.path.onMouseEnter = function(e) {
			// this.fillColor = this.selected ? 'red' : 'orange';
		}

		letterPoint.path.onMouseLeave = function(e) {
			// this.fillColor = 'white';
		}

		letterPoint.path.onMouseDown = function(e) {
		...
		letterPoint.path.onKeyDown = function(e) {
			console.log(e.key);
			switch(e.key) {
				case 'delete' :
					console.log('de');
