...
LetterView.prototype = {
	...
	draw : function(letterModel, renderFunc) {
		...
		$.each(letterModel.points, function(idx, point) {
			...
			var path = letter.createLetterPoint(grid.renderFunc(point), idx, letterModel.selectedPoints.indexOf(idx) > -1, letterModel.transforms[idx] || null);

			if(idx) {
				letterPath.lineTo(path.x, path.y);
			} else {
				letterPath.moveTo(path);
			}
	...
	createLetterPoint: function(point, value, selected, transform) {
		...
		if(transform)
			path.add((function() { var point = new paper.Point(); point.angle = transform[0]; point.length = transform[1]; return point;})());
...
RuneView.prototype = {
	...
	drawLetter : function(letter) {
		...
		this.letter.draw(letter, this.grid.renderPoint);
