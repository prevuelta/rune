...
Transform.prototype = {
	...
	weight : function(points, res) {
		...
		var tempPoints = points.map(function(entry) {
...
// 				points[1].y = points[2].y - finalMeasure;
		...
		var midPoint = tempPoints[0].getMid(points[2]);
		...
		// Hypothesis to midpoint
		var t1_hyp = tempPoints[2].getDistance(midPoint);
		...
		// var vec = new paper.Point(tempPoints[2]);
		var vec = new paper.Point();

		vec.angle = (90 - trigUtil.radToDeg( trigUtil.getAngle(tempPoints[0], tempPoints[2]))) - t1_phi;

		var side = trigUtil.getSize(null, t1_adj, t1_hyp);

		vec.length = side;

		var tangentPoint = tempPoints[2].subtract(vec);
	 
		testPath.moveTo(tempPoints[2]);
		...
		var otherPoint = new paper.Point(tempPoints[0].x, tempPoints[2].y);

		// Distance between tempPoints[2] and 0 on y axis
		var t2_adj = otherPoint.getDistance(tempPoints[2]);
		...
		// var newPoint = tempPoints[2].subtract(finalVector.length);
		...
		var finalMeasure = tempPoints[0].getDistance(newPoint3);
